...
import { PYTHON_PROBLEMS } from "../data/pythonProblemsData";
import { sfx } from "../utils/sound";

export default function PythonLearnPage({ onBack, user, onUpdateUser }) {
  const [filter, setFilter] = useState("All"); // All | Easy | Medium | Hard
  const [search, setSearch] = useState("");
  const [activeProblem, setActiveProblem] = useState(PYTHON_PROBLEMS[0]);
  const [solvedList, setSolvedList] = useState([]);
  const [copied, setCopied] = useState(false);

  const filteredProblems = PYTHON_PROBLEMS.filter((p) => {
    ...
    <div className="min-h-screen bg-[#080c14] text-white pb-16">
      ...
      <div className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <button
            onClick={onBack}
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-amber-500/40 text-xs font-bold text-slate-300 hover:text-white transition-all cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> Return to Dashboard
          </button>

          <div className="flex items-center gap-2">
            <span className="text-xs font-mono px-2.5 py-1 rounded-md bg-amber-950/60 border border-amber-500/30 text-amber-400 font-bold">
              50 Python Hunter Trials
            </span>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-3">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search Python problem title or category..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-amber-500 placeholder:text-slate-600"
              />
            </div>
              ...
              return (
                <div
                  ...
                  className={`p-3.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${
                    isSelected
                      ? "bg-slate-900 border-amber-500/70 shadow-[0_0_15px_rgba(245,158,11,0.2)]"
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-5 shadow-2xl">
            ...
            <div className="flex items-start justify-between border-b border-slate-800 pb-4">
              <div>
                <span className="text-[10px] uppercase font-mono tracking-widest px-2.5 py-1 rounded bg-amber-950 border border-amber-500/30 text-amber-300 font-black">
                  PYTHON {activeProblem.difficulty} ARCHIVE •{" "}
            ...
            <div className="rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden shadow-inner">
              ...
              <pre className="p-4 text-xs font-mono text-amber-300/95 leading-relaxed overflow-x-auto selection:bg-amber-500/30">
